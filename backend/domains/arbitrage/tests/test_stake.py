...
from sports_quant.backend.domains.arbitrage.models import StakeMethod
from sports_quant.backend.domains.arbitrage.stake import (
...
class TestDistributeStakes:
    ...
    def test_weighted_same_as_equal(self):
        eq = distribute_stakes(
            [Decimal("2.0"), Decimal("3.0")],
            method=StakeMethod.EQUAL_PROFIT,
        )
        w = distribute_stakes(
            [Decimal("2.0"), Decimal("3.0")],
            method=StakeMethod.WEIGHTED,
        )
        assert eq == w
    ...
    def test_risk_parity_same_as_equal(self):
        ...
        rp = distribute_stakes(
            [Decimal("2.0"), Decimal("3.0")],
            method=StakeMethod.RISK_PARITY,
        )
        assert eq == rp
    ...
    def test_with_max_stakes_no_cap(self):
        """When max_stakes are much larger than needed, no capping."""
        stakes = distribute_stakes(
            [Decimal("2.0"), Decimal("3.0")],
            total_stake=Decimal("100"),
    ...
    def test_with_max_stakes_capped(self):
        """When one leg hits its max, redistribute to the other."""
        stakes = distribute_stakes(
            [Decimal("2.0"), Decimal("3.0")],
            total_stake=Decimal("100"),
            commissions=None,
...
class TestRequiredStakeForReturn:
    ...
    def test_with_commission(self):
        stake = required_stake_for_return(
            Decimal("200"), Decimal("2.0"), Decimal("0.05"),
        )
        assert stake > Decimal("100.00")  # need more stake due to commission

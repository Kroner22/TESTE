...
from sports_quant.backend.domains.arbitrage.models import (
    BookmakerOdds, ArbType, ArbGrade, ArbFilter,
)
from sports_quant.backend.domains.arbitrage.core import (
...
class TestArbProfitPct:
    ...
    def test_with_commissions(self):
        ...
        assert pct < arb_profit_pct(odds)  # commission reduces profit
...
class TestBestOddsAcrossBookmakers:
    ...
    def test_with_commission(self):
        ...
        best = best_odds_across_bookmakers(odds_by_outcome)
        # B's odd 2.1 / 1.10 = 1.909, A's odd 2.0 / 1.0 = 2.0  → A wins
...
class TestGradeArbitrage:
    def test_elite(self):
        assert grade_arbitrage(Decimal("0.07")) == ArbGrade.ELITE

    def test_strong(self):
        assert grade_arbitrage(Decimal("0.04")) == ArbGrade.STRONG

    def test_solid(self):
        assert grade_arbitrage(Decimal("0.02")) == ArbGrade.SOLID

    def test_marginal(self):
        assert grade_arbitrage(Decimal("0.008")) == ArbGrade.MARGINAL

    def test_noise(self):
        assert grade_arbitrage(Decimal("0.003")) == ArbGrade.NOISE
...
class TestDetect2WayArb:
    def test_detects_arb(self):
        odds_by_outcome = {
            "home": [
                BookmakerOdds(bookmaker="A", outcome="home", odd=Decimal("2.1")),
            ],
            "away": [
                BookmakerOdds(bookmaker="B", outcome="away", odd=Decimal("2.2")),
            ],
        }
        arb = detect_2way_arb(
    ...
    def test_no_arb(self):
        odds_by_outcome = {
            "home": [
                BookmakerOdds(bookmaker="A", outcome="home", odd=Decimal("1.3")),
            ],
            "away": [
                BookmakerOdds(bookmaker="B", outcome="away", odd=Decimal("3.5")),
            ],
        }
    ...
    def test_filtered_by_min_profit(self):
        odds_by_outcome = {
            "home": [
                BookmakerOdds(bookmaker="A", outcome="home", odd=Decimal("2.01")),
            ],
            "away": [
                BookmakerOdds(bookmaker="B", outcome="away", odd=Decimal("2.02")),
            ],
        }
    ...
    def test_actionable_flag(self):
        odds_by_outcome = {
            "home": [
                BookmakerOdds(bookmaker="A", outcome="home", odd=Decimal("2.1"), max_stake=Decimal("10")),
            ],
            "away": [
                BookmakerOdds(bookmaker="B", outcome="away", odd=Decimal("2.2"), max_stake=Decimal("1000")),
            ],
        }
        arb = detect_2way_arb(
            "evt1", "tennis", "A", "B", "h2h",
        ...
        # home leg needs > 10, so not actionable
        assert arb is not None
...
class TestDetect3WayArb:
    def test_detects_arb(self):
        odds_by_outcome = {
            "home": [
                BookmakerOdds(bookmaker="A", outcome="home", odd=Decimal("3.5")),
            ],
            "draw": [
                BookmakerOdds(bookmaker="B", outcome="draw", odd=Decimal("3.8")),
            ],
            "away": [
                BookmakerOdds(bookmaker="C", outcome="away", odd=Decimal("4.0")),
            ],
        }
        arb = detect_3way_arb(
